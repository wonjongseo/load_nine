...
def migrate_default_hunting_v4(config: dict) -> None:
    ...
    기존 분면별 override가 있으면 새 step id로 이전하며,
    이미 새 step id로 저장된 값은 절대 덮어쓰지 않습니다.
    """
    ...
    old_to_new = {
        "18_monster": "17_monster",
        "19_monster": "17_monster",
        "19_quick_move": "18_quick_move",
        "20_confirm": "19_confirm",
        "21_auto": "20_auto",
        "22_auto": "20_auto",
    }
    ...
    for target in config.get("targets", {}).values():
        ...
        # 이미 새 id로 저장된 값이 있으면 그 값을 최우선으로 보존.
        for old_id, new_id in old_to_new.items():
            if new_id not in overrides and old_id in overrides:
                overrides[new_id] = overrides[old_id]

            if new_id not in fallbacks and old_id in fallbacks:
                fallbacks[new_id] = fallbacks[old_id]
